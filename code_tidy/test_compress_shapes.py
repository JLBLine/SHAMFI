...
if __name__ == '__main__':
    ...
    n1s_high,n2s_high,coeffs_high,n1s_low,n2s_low,coeffs_low = compress_coeffs(n1s,n2s,coeffs,num_coeffs,xrot,yrot,b1,b2)

    A_shape_basis_high = gen_A_shape_matrix(n1s=n1s_high,n2s=n2s_high,xrot=xrot,yrot=yrot,b1=b1,b2=b2)

    coeffs_high.shape = (len(coeffs_high),1)
    fit_data_high = fitted_model(coeffs=coeffs_high,A_shape_basis=A_shape_basis_high)

    A_shape_basis_low = gen_A_shape_matrix(n1s=n1s_low,n2s=n2s_low,xrot=xrot,yrot=yrot,b1=b1,b2=b2)

    coeffs_low.shape = (len(coeffs_low),1)
    fit_data_low = fitted_model(coeffs=coeffs_low,A_shape_basis=A_shape_basis_low)


    print('Sum high', sum(fit_data_high))
    print('Sum low', sum(fit_data_low))
    ...
    try:
        fit_data_high.shape = data.shape
    except:
        fit_data_high.shape = ras_mesh.shape
    factor = 0.0001
    fit_data_high[where(abs(fit_data_high) < fit_data_high.max()*factor)] = NaN
    current_cmap = matplotlib.cm.get_cmap()
    current_cmap.set_bad(color='grey')
    if args.plot_lims:
        vmin,vmax = map(float,args.plot_lims.split(','))
        im1 = ax1.imshow(fit_data_high,vmin=vmin,vmax=vmax,origin='lower')
    else:
        im1 = ax1.imshow(fit_data_high,origin='lower')

    # ax1.contour(dataset0,colors='w',alpha=0.3)
    add_colourbar(fig=fig,im=im1,ax=ax1)
    ax1.set_title('Highest rank coeffs')

    ax1.axvline(floor(fit_data_high.shape[1]/2),color='w',alpha=0.3,lw=0.5)
    ax1.axhline(floor(fit_data_high.shape[0]/2),color='w',alpha=0.3,lw=0.5)

    try:
        fit_data_low.shape = data.shape
    except:
        fit_data_low.shape = ras_mesh.shape
    factor = 0.0001
    fit_data_low[where(abs(fit_data_low) < fit_data_low.max()*factor)] = NaN
    current_cmap = matplotlib.cm.get_cmap()
    current_cmap.set_bad(color='grey')
    if args.plot_lims:
        vmin,vmax = map(float,args.plot_lims.split(','))
        im1 = ax2.imshow(fit_data_low,vmin=vmin,vmax=vmax,origin='lower')
    else:
        im1 = ax2.imshow(fit_data_low,origin='lower')

    # ax2.contour(dataset0,colors='w',alpha=0.3)
    add_colourbar(fig=fig,im=im1,ax=ax2)
    ax2.set_title('lowest rank coeffs')

    ax2.axvline(floor(fit_data_low.shape[1]/2),color='w',alpha=0.3,lw=0.5)
    ax2.axhline(floor(fit_data_low.shape[0]/2),color='w',alpha=0.3,lw=0.5)

    plt.tight_layout()
    fig.savefig('%s_compress.png' %(args.srclist.split('/')[-1][:-4]), bbox_inches='tight')
